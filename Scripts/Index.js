...
function checkForUpdates(idForTimer){
	if(idForTimer == "sortie0" || idForTimer == "sortie1" || idForTimer ==  "sortie2"){
		...
		setUpDailyReset();
	}else if(idForTimer == "alert0" || idForTimer == "alert1" || idForTimer == "alert2" || idForTimer == "alert3" || idForTimer == "alert4"
		|| idForTimer == "alert5" || idForTimer == "alert6" || idForTimer == "alert7" || idForTimer == "alert8" || idForTimer == "alert9"){
		$('#Alerts').empty();
		setUpAlerts();
	}else if(idForTimer == "void0" || idForTimer == "void1" || idForTimer == "void2" || idForTimer == "void3" || idForTimer == "void4"
		|| idForTimer == "void5" || idForTimer == "void6" || idForTimer == "void7" || idForTimer == "void8" || idForTimer == "void9"){
		$('#Void').empty();
		setUpVoid();
...
function setUpSortie(){
	var url = 'https://api.warframestat.us/ps4/sortie';
	var zDdata = {};
	$.getJSON(url, zDdata, buildSortieTable);
}

var buildSortieTable = function(data) {
	$('#Sortie').empty();
...
//Cetus
function setUpCetus(){
	var url = 'https://api.warframestat.us/ps4/cetusCycle';
	var zDdata = {};
	$.getJSON(url, zDdata, buildCetusTable);
}

var buildCetusTable = function(data){
	$('#Cetus').empty();
	var end = new Date(data.expiry);
...
function setUpDailyReset(){
	var url = 'https://api.warframestat.us/ps4/sortie';
	var zDdata = {};
	$.getJSON(url, zDdata, buildDailyResetTable);
}
var buildDailyResetTable = function(data){
	$('#Daily').empty();
	...
	//before reset
	if(sortieEnd - today > 57600000){ //16hrs till next sortie reset then need different math
...
function setUpAlerts(){
	var url = 'https://api.warframestat.us/ps4/alerts';
	var zDdata = {};
	$.getJSON(url, zDdata, buildAlertsTable);
}

var buildAlertsTable = function(data){ //used to build table
	...
	setInterval(function() {
		var changes = false;
		$.getJSON('https://api.warframestat.us/ps4/alerts', function(alertData) {
...
// Void Fissures
function setUpVoid(){
	var url = 'https://api.warframestat.us/ps4/fissures';
	var zDdata = {};
	$.getJSON(url, zDdata, buildVoidTable);
}

function buildVoidTable(voidData){
	voidJSON = voidData;
	$('#Void').empty();
	...
	setInterval(function() {
		var changes = false;
		$.getJSON('https://api.warframestat.us/ps4/fissures', function(voidData) {
...
//darvo
function setUpDarvo(){
	var url = 'https://api.warframestat.us/ps4/dailyDeals';
	var zDdata = {};
	$.getJSON(url, zDdata, buildDarvoTable);
}

function buildDarvoTable(darvoData){
	$('#Darvo').empty();
...
//news
function setUpNews(){
	var url = 'https://api.warframestat.us/ps4/news';
	var zDdata = {};
	$.getJSON(url, zDdata, buildNewsTable);
}
...
function setUpSyndicate(){
	var url = 'https://api.warframestat.us/ps4/syndicateMissions';
	var zDdata = {};
	$.getJSON(url, zDdata, buildSyndicateTable);
}

function buildSyndicateTable(synData){
	$('#Syndicate').empty();
	var expSyn = new Date(synData[0].expiry);
	for(var i = 0; i <synData.length; i++){
		var nodes = "";
		var singleNode = "";
		if(synData[i].syndicate != "Quills" && synData[i].syndicate != "Operations Syndicate" && synData[i].syndicate != "Ostrons" && synData[i].syndicate != "Assassins" && synData[i].syndicate != "EventSyndicate"){
			singleNodes = synData[i].nodes;
			for(var j = 0; j<singleNodes.length -1;j++){
				nodes += singleNodes[j] + " | ";
			}
			nodes += singleNodes[singleNodes.length -1];
			$('#Syndicate').append("<div id =\"wrap\"> <p id = \"white\">"+ nodes +"</p><p class = synTimer id =\"syn" + i +"\">...</p><p class=\"syn\">"+ synData[i].syndicate + "</p></div>");
			var timer = "syn"+i;
...
//Baro Ki'Teer
function setUpBaro(){
	var url = 'https://api.warframestat.us/ps4/voidTrader';
	var zDdata = {};
	$.getJSON(url, zDdata, buildBaroTable);
}

function buildBaroTable(baroData){
	$('#Baro').empty();
	var baroStart = new Date(baroData.activation);
	var baroEnd = new Date(baroData.expiry);
	var now = new Date();
	if(baroStart - now >0){ //hasn't started
		$('#Baro').append("<div id =\"wrap\"><p id=\"voidTrader\">" + baroData.character + " | Coming to: " + baroData.location +"</p><p class = \"baroTimer\" id=\"baro\">...</p></div>");
		setTimer(baroStart,'baro',0, 'start');
	}else{ //it started
		$('#Baro').append("<div id =\"wrap\"><p id=\"voidTrader\">" + baroData.character + " | Located at: " + baroData.location +"</p><p class = \"baroTimer\" id=\"baro\">...</p></div>");
		setTimer(baroEnd, 'baro',0, "active");
		for(var i = 0; i<baroData.inventory.length; i++){
			$('#Baro').append("<div id = \"wrap\"><p id=\"white\">" + baroData.inventory[i].item + "</p> <p class = \"price\">"+ baroData.inventory[i].ducats + " ducats | "
				+ baroData.inventory[i].credits + " credits" +"</p></div>")
		}
	}	
}
//------------------------------------------End of Baro Ki'Teer----------------------------------------------------------------------------------
...
function setUpInvasions(){
	var url = 'https://api.warframestat.us/ps4/invasions';
	var zDdata = {};
	$.getJSON(url, zDdata,buildInvasionTable);
}

function buildInvasionTable(invData){
	$('#Invasions').empty();
		...
		if(percent >=0 && percent <100){
			if(invData[i].attackerReward.countedItems.length > 0 ){
				if(invData[i].attackerReward.countedItems[0].count > 1){ //more then 1 reward of same time
					$('#Invasions').append("<div class = \"invasions\"><br><p class = \"invasionsNode\">"+ invData[i].node + " | " + invData[i].attackingFaction + " vs " 
						+ invData[i].defendingFaction + " | " + percent + "% left"+"</p> <p class = \"invasionsGreenLeft\" id=\"leftDrop"+i+"\">"+invData[i].attackerReward.countedItems[0].count 
						+ " " + invData[i].attackerReward.countedItems[0].type + "</p><p class = \"invasionsGreenRight\" id=\"rightDrop"+i+"\">"+ invData[i].defenderReward.countedItems[0].count + " " 
						+ invData[i].defenderReward.countedItems[0].type + "</p> <div  id=\"myProgress"+i
						+"\" style=\"width:99%;background-color:red;border-radius:10px;margin:0px 0px 10px 3px;position:absolute;top:25px;\"><div id=\"container\"><img id=\"left"
						+i+"\" + src=\"./grineer.png\" style=\"height:33px;position:absolute;left:5px;z-index:5;top:-1px;\"><img id=\"right"
						+i+"\" + src=\"./grineer.png\" style=\"height:33px;position:absolute;right:6px;top:-1px;\"></div><div id=\"myBar"+i +"\" style=\"width:"
						+ percent+"%;height:30px;background-color:green;border-radius:10px\"></div></div></div>");
				}
				else{ // 1 reward
					$('#Invasions').append("<div class = \"invasions\"><br><p class = \"invasionsNode\">"+ invData[i].node + " | " + invData[i].attackingFaction + " vs " 
						+ invData[i].defendingFaction + " | " + percent + "% left"+"</p> <p class = \"invasionsGreenLeft\" id=\"leftDrop"+i+"\">"+ invData[i].attackerReward.countedItems[0].type 
						+ "</p><p class = \"invasionsGreenRight\" id=\"rightDrop"+i+"\">"	+ invData[i].defenderReward.countedItems[0].type + "</p> <div id=\"myProgress"+i
						+"\" style=\"width:99%;background-color:red;border-radius:10px;margin:0px 0px 10px 3px;position:absolute;top:25px;\"><div id=\"container\"><img id=\"left"
						+i+"\" + src=\"./grineer.png\" style=\"height:33px;position:absolute;left:5px;z-index:5;top:-1px;\"><img id=\"right"
						+i+"\" + src=\"./grineer.png\" style=\"height:33px;position:absolute;right:6px;top:-1px;\"></div><div id=\"myBar"+i 
						+"\" style=\"width:"+ percent+"%;height:30px;background-color:green;border-radius:10px\"></div></div></div>");
				...
				if(invData[i].defenderReward.countedItems[0].count > 1){ // 1 reward with multiple of it
					$('#Invasions').append("<div class = \"invasions\"><br><p class = \"invasionsNode\">"+ invData[i].node + " | " + invData[i].attackingFaction + " vs " 
							+ invData[i].defendingFaction + " | " + percent + "% left"+"</p> <p class = \"invasionsGreenRight\" id=\"rightDrop"+i+"\">"+ invData[i].defenderReward.countedItems[0].count + " " 
							+ invData[i].defenderReward.countedItems[0].type + "</p> <div  id=\"myProgress"+i
							+"\" style=\"width:99%;background-color:red;border-radius:10px;margin:0px 0px 10px 3px;position:absolute;top:25px;\"><div id=\"container\"><img id=\"left"
							+i+"\" + src=\"./grineer.png\" style=\"height:33px;position:absolute;left:5px;z-index:5;top:-1px;\"><img id=\"right"
							+i+"\" + src=\"./grineer.png\" style=\"height:33px;position:absolute;right:6px;top:-1px;\"></div><div id=\"myBar"+i 
							+"\" style=\"width:"+ percent+"%;height:30px;background-color:green;border-radius:10px\"></div></div>");				
				}else{
					$('#Invasions').append("<div class = \"invasions\"><br><p class = \"invasionsNode\">"+ invData[i].node + " | " + invData[i].attackingFaction + " vs " 
							+ invData[i].defendingFaction + " | " + percent + "% left"+"</p> <p class = \"invasionsGreenRight\" id=\"rightDrop"+i+"\">"+ invData[i].defenderReward.countedItems[0].type 
							+ "</p> <div  id=\"myProgress"+i +"\" style=\"width:99%;background-color:red;border-radius:10px;margin:0px 0px 10px 3px;position:absolute;top:25px;\"><div id=\"container\"><img id=\"left"
							+i+"\" + src=\"./grineer.png\" style=\"height:33px;position:absolute;left:5px;z-index:5;top:-1px;\"><img id=\"right"
							+i+"\" + src=\"./grineer.png\" style=\"height:33px;position:absolute;right:6px;top:-1px;\"></div><div id=\"myBar"+i 
							+"\" style=\"width:"+ percent+"%;height:30px;background-color:green;border-radius:10px\"></div></div>");
